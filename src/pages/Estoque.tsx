...
const emptyForm: Omit<InventoryItem, "id" | "created_at" | "updated_at" | "organization_id"> = {
  nome: "",
  categoria: null,
  quantidade: 0,
  unidade: "un",
...
export default function Estoque() {
  ...
  const stats = {
    ...
    baixoEstoque: items.filter(
      (i) => i.estoque_minimo && i.quantidade <= i.estoque_minimo && i.quantidade > 0
    ).length,
    semEstoque: items.filter((i) => i.quantidade === 0).length,
    valorTotal: items.reduce(
      (sum, i) => sum + (i.preco_unitario || 0) * i.quantidade,
      0
    ),
  };
                  ...
                  {filteredItems.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="text-right">
                          {item.preco_unitario
                            ? `R$ ${item.preco_unitario.toLocaleString("pt-BR", {
                        ...
                        <TableCell className="text-right">
                          {item.preco_unitario
                            ? `R$ ${(item.preco_unitario * item.quantidade).toLocaleString(
                                "pt-BR",
                                { minimumFractionDigits: 2 }
                              )}`
                            : "-"}
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm({
                      ...form,
                      estoque_minimo: e.target.value ? parseFloat(e.target.value) : null,
                    })
                  }
